...
; (function (global, factory) {
	...
	return (function (ns) {
		...
		var isDebug = (typeof window === 'undefined') ? true : window.location.href.search(/local/i) > 0;
		logger['trace'] = defineScopedTracing("logger", isDebug);
		ns.modules.logger = logger;
		ns.modules.logger.trace.debug(logger.version);

		return temp = ns.modules.logger;

		// both of these dependencies are resolved in string.js
	})(global.spexplorerjs = (typeof global.spexplorerjs === 'undefined') ? { modules: {} } : global.spexplorerjs);
}));

export = temp.defineScopedTracing;