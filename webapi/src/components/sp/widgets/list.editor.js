...
(function(ns, $, template) {
	...
	var SPListWidget = function(el/*, opts*/) {// constructor
		...
		var query = ns.widgets.querybuilder.startup($el).data("xwidget");
		...
		ns.widgets.xSPTreeLight.startup($(".listSelectorFirst", $el)).on("selectionchange", function(event, list) {
			...
			if (list.isPropertyAvailable("SchemaXml")) {
				...
				ns.modules.spapi.loadSpElem(list, ctx).done(showSchema);
			}
		});
	...
	ns.widgets.addSpWidget("spListWidget", SPListWidget, "0.1.7");
